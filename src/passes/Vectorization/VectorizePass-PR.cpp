...
static bool isReverse(llvm::MutableArrayRef<ScalarAssignGroup> &group) {
  if (group.size() < 2)
    return false;
  int dstStride = group[1].dstIndex - group[0].dstIndex; // expected  +1
  int srcStride = group[1].srcIndex - group[0].srcIndex; // expected  -1
...
void vectorizeReverseGroup(llvm::MutableArrayRef<ScalarAssignGroup> group,
  ...
  mlir::Location loc = group.front().assign.getLoc();
  ...
  // type info 
  auto srcIntTy = mlir::cast<moore::IntType>(srcVec.getType());
  auto domain   = srcIntTy.getDomain();  // TwoValued / FourValued domain

  // bit and vector types for extraction and concatenation
  auto bitType   = moore::IntType::get(builder.getContext(), 1, domain);
  auto sliceType = moore::IntType::get(builder.getContext(), width, domain);

  // extract bits in ascending order 
  SmallVector<Value> bits;
  for (int i = lowBit; i <= highBit; ++i){
    bits.push_back(builder.create<moore::ExtractOp>(loc, bitType, srcVec, builder.getI32IntegerAttr(i)));
  }
  ...
  builder.create<moore::ContinuousAssignOp>(loc, dstVec, reversedVec);
...
void processIndexMap(IndexedGroupMap &indexMap, mlir::Value dst, mlir::Value src) {
  ...
  if (group.size() > 1) {
    if(isReverse(group)){
      vectorizeReverseGroup(group, dst, src);
    }
    else vectorizeGroup(group, dst, src);
  }
}
